...
def method_weight_func(method) : 
    ...
    for i in range(1,len(methods)):
        signal_output_weight1 = (signal_output_quantile[methods].loc[0.5].loc[methods.iloc[i]] < signal_output[methods.iloc[1]]) & (signal_output_quantile[methods].loc[0.75].loc[methods.iloc[i]] < signal_output[methods.iloc[1]]).replace[{True = 1, False =0}]
        signal_output_weight2 = (signal_output_quantile[methods].loc[0.75].loc[methods.iloc[i]] < signal_output[methods.iloc[1]]).replace[{True = 1, False =0}]
        signal_output[str(method.loc[i]) + '_weight'] = signal_output_weight.replace({True : 1 , False : 0})
        
    signal_output['Max_method'] = signal_output[methods].sum(axis=1)
