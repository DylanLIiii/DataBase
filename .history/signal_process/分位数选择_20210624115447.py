...
def method_weight_func(method) : 
    ...
    for i in range(1,len(methods)):
        signal_output_weight1 = (signal_output_quantile[methods].loc[0.5].loc[methods.iloc[i]] < signal_output[methods.iloc[1]]) & (signal_output_quantile[methods].loc[0.75].loc[methods.iloc[i]] < signal_output[methods.iloc[1]]).rep
