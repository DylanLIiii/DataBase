...
methods = pd.Series(['annual_return_sp','win_rate_sp','pc_ratio_sp'])
for i in range(1,len(methods)):
    def get_weight_group(x) :
        if x < signal_output_quantile[methods].loc[0.25].loc[methods.iloc[i]]: 
            return 1
        elif signal_output_quantile[methods].loc[0.25].loc[methods.iloc[i]] < x < signal_output_quantile[methods].loc[0.5].loc[emthods.iloc[1]]: 
            return 2 
