...
signal_output.
### 得到信号
signal_output_quantile = signal_input[['annual_return_sp','win_rate_sp','pc_ratio_sp']].quantile([0.25,0.5,0.75])
for col_name in signal_output[['annual_return_sp','win_rate_sp','pc_ratio_sp']]
    def ge_weight_group(x) :
    if x < signal_output_quantile[col_name].loc[0.25]: 
        return 1
    elif signal_output_quantile[col_name].loc[0.25] < x  and  x < signal_input_quantile[col_name].loc[0.5]: 
        return 2 
