...
def get_weight_group(x) :
    if x < signal_output_quantile[method].loc[0.25].loc[method]: 
        return 1
    elif signal_output_quantile[method].loc[0.25].loc[method] < x < signal_output_quantile[method].loc[0.5].loc[method.iloc[1]]: 
        return 2 
    elif signal_output_quantile[method].loc[0.5].loc[method] < x < signal_output_quantile[method].loc[0.75].loc[method.iloc[1]] : 
        return 3         
    else : 
        return 4
