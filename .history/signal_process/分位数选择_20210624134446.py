...
def method_weight_func(method) : 
    ...
    for i in range(1,len(methods)):
        def get_weight_group(x) :
            if x < signal_output_quantile[methods].loc[0.25].loc[methods.iloc]: 
                return 1
            elif signal_output_quantile[methods].loc[0.25].loc[methods.iloc] < x < signal_output_quantile[methods].loc[0.5]: 
                return 2 
            elif signal_output_quantile[methods].loc[0.5].loc[emthods.iloc] < x < signal_output_quantile[methods].loc[0.75].loc[methods.iloc[1]] : 
                return 3         
            else : 
                return 4
        signal_output_weight1 = (signal_output_quantile[methods].loc[0.5].loc[methods.iloc] < signal_output[methods.iloc[1]]) & (signal_output_quantile[methods].loc[0.75].loc[methods.iloc] < signal_output[methods.iloc[1]])
        signal_output_weight1.replace({True : 1, False : 0})
        signal_output_weight2 = (signal_output_quantile[methods].loc[0.75].loc[methods.iloc] < signal_output[methods.iloc[1]])
        signal_output_weight2.replace({True : 2, False : 0})
        signal_output_weight = signal_output_weight2 + signal_output_weight1
        signal_output[str(method.loc) + '_weight'] = signal_output_weight1 + signal_output_weight2
