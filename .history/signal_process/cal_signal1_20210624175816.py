...
signal_output_['Max_method'] = signal_input.sort_values(by = 'Max_method', ascending = False ).groupby('start_date').head(3)['Max_method'].nlargest(3, keep  = 'all')

 #当期相同则计算上期method
if len(signal_output_['Max_method']) > 3:       
    signal_output_.sort_values(by=['model','start_date'],inplace=True) 
    signal_output_['Last_method'] = signal_output_['Max_method'].shift(1) 
    signal_output = signal_output_.nlargest(3,'Last_method',keep="first")
else:
    signal_output = signal_output_.sort_values(by="start_date").shift(1)
